...
class EasyHttp_Curl
{
	...
	function request($url, $args = array())
	{
		...
		if (0 == strlen($theResponse) && empty($theHeaders['headers'])) {
			if ($curl_error = curl_error($handle) && EasyHttp_Proxy::$host !== null)
			{
				// echo "============No response. Try to request without proxy===========\r\n";
				EasyHttp_Proxy::$host = EasyHttp_Proxy::$port = null;
				$r = $this->request($url, $args);
				if(is_array($r))
					$r['proxy'] = false;
				else if(is_object($r))
					$r->proxy = false;
				return $r;
			}else if ($curl_error = curl_error($handle))
